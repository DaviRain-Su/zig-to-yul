...
| Gas 估算 | 预估交易 gas 消耗 | ⚠️ 增强实现（内存扩展/冷热访问/SSTORE 估算/新账户成本/退费估算/常量表达式折叠/访问列表输入/分支权重标注/循环推断/循环覆盖/栈深度估计/退费按版本细分） |
| Yul 优化器 | 内置 Yul 级别优化 | ✅ 扩展实现（常量折叠/控制流化简/无效语句移除/恒等式简化） |
