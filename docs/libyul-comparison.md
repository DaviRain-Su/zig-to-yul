...
| 事件解码 (Event Decoding) | 解析合约事件日志 | ⚠️ 基础实现（静态类型/bytes/string/数组/动态数组元素/tuple/数组 tuple/CLI/indexed 预镜像/签名校验/匿名事件） |
...
| 节点类型 | 状态 | Yul 对应 |
|----------|------|----------|
| for 循环 | ⚠️ 部分 | 支持 `for (start..end)` 与 `for (start..)` (payload 可为 `_`) |
| while 循环 | ✅ | `for { } cond { } { }` |
