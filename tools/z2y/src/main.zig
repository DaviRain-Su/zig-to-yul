...
fn generateAbiWrapper(
    ...
    for (parsed.value.array.items) |entry| {
        ...
        const inputs = try parseAbiTypes(allocator, obj.get("inputs"));
    ...
    try out.appendSlice(allocator, "const evm = @import(\"evm\");\n\n");
...
fn emitFunction(allocator: std.mem.Allocator, out: *std.ArrayList(u8), func: AbiFunction, module_name: []const u8) !void {
    ...
    if (func.inputs.len == 0) {
        ...
        for (func.inputs, 0..) |input, idx| {
            const zig_type = try zigTypeForAbi(allocator, input);
            defer allocator.free(zig_type);
            try out.appendSlice(allocator, "arg");
            try out.appendSlice(allocator, try std.fmt.allocPrint(allocator, "{d}", .{idx}));
            try out.appendSlice(allocator, ": ");
...
fn emitFunctionBody(allocator: std.mem.Allocator, out: *std.ArrayList(u8), func: AbiFunction, module_name: []const u8) !void {
    _ = module_name;
    const signature = try buildSignature(allocator, func.name, func.inputs);
    defer allocator.free(signature);

    try out.appendSlice(allocator, "        const selector = evm.abi.selectorWord(\"");
    try out.appendSlice(allocator, signature);
    try out.appendSlice(allocator, "\");\n");
    try out.appendSlice(allocator, "        mstore(0x00, selector);\n");

    for (func.inputs, 0..) |input, idx| {
        try emitArgStore(allocator, out, input, idx);
    }
    ...
    try out.appendSlice(allocator, "        const ok = ");
    try out.appendSlice(allocator, call_kind);
    if (std.mem.eql(u8, call_kind, "staticcall")) {
        try out.appendSlice(allocator, "(gas(), self.address, 0x00, ");
    } else {
        try out.appendSlice(allocator, "(gas(), self.address, 0, 0x00, ");
    }
    try out.appendSlice(allocator, try std.fmt.allocPrint(allocator, "0x{x}", .{input_len}));
    try out.appendSlice(allocator, ", 0x00, ");
    try out.appendSlice(allocator, try std.fmt.allocPrint(allocator, "0x{x}", .{output_len}));
    try out.appendSlice(allocator, ");\n");
    try out.appendSlice(allocator, "        if (ok == 0) revert(0, 0);\n");
...
fn emitArgStore(allocator: std.mem.Allocator, out: *std.ArrayList(u8), abi_type: []const u8, index: usize) !void {
    ...
    if (std.mem.eql(u8, abi_type, "uint256") or std.mem.eql(u8, abi_type, "uint") or std.mem.eql(u8, abi_type, "address") or std.mem.eql(u8, abi_type, "bool")) {
        try out.appendSlice(allocator, "        mstore(");
        try out.appendSlice(allocator, try std.fmt.allocPrint(allocator, "0x{x}", .{offset}));
        try out.appendSlice(allocator, ", arg");
        try out.appendSlice(allocator, try std.fmt.allocPrint(allocator, "{d}", .{index}));
        try out.appendSlice(allocator, ");\n");
        return;
    }
...
fn emitLoadDecode(allocator: std.mem.Allocator, out: *std.ArrayList(u8), abi_type: []const u8, index: usize) !void {
    ...
    const load_expr = try std.fmt.allocPrint(allocator, "mload(0x{x})", .{offset});
